...
add_shortcode('generic-email', 'generic_email_shortcode');
    ...
    if (isset($_POST['submit-subscription'])) {
        // global $wpdb;
        $db_table = 'wp_generic_emails';
        $email          = $_POST['email'];
        $newsletter_1   = (empty($_POST['checkbox_1'])) ? 'empty' : $_POST['checkbox_1']; 
        $data   = array(
                    'email'             => $email,
                    'secondary_optin'   => $newsletter_1,
                    'created_at'        => date("d-m-y H:i:s"),
                    'updated_at'        => ''
                );
        $arrayName = array('%s', '%s');

        $results = $wpdb->insert('wp_generic_emails', $data, $format );
